...
const Player = () => {
  const { current, playlist, isPlaying, togglePlay, Next, Prev } = usePlayer();
  ...
  return (
    <div
      className={twMerge(
        "fixed left-[2.5%] z-10 -bottom-16 transition-all duration-300  h-16 w-[80%] bg-[#0F172A99] backdrop-blur-lg rounded-lg flex items-center px-2 justify-between",
    ...
    >
      ...
      <div
        className={twMerge(
          ...
          !!current && fullScreen && "flex-col w-[85vw] max-w-[400px] ",
        )}
        onClick={() => {
          setFullScreen((prev) => !prev);
        }}
      >
        ...
        />
        <div className={twMerge("font-sans", fullScreen && "text-center mt-3")}>
      ...
      >
        ...
        <div className="flex justify-between mt-3">
          <div>{new Date(timeElapsed * 1000).toISOString().substr(11, 8)}</div>
          <div>
            <div>
              {new Date((audio && audio.duration ? audio.duration : 0) * 1000)
                .toISOString()
                .substr(11, 8)}
            </div>
          </div>
